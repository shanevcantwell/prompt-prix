"""
PooledLocalInferenceAdapter - Generic OpenAI-compatible pooled inference adapter.

Uses local-inference-pool for multi-server slot management, manifest refresh,
...
class LocalInferenceError(Exception):
    """Human-readable error from a local OpenAI-compatible inference server."""
    pass
...
# ─────────────────────────────────────────────────────────────────────
# PUBLIC: PooledLocalInferenceAdapter
# ─────────────────────────────────────────────────────────────────────

class PooledLocalInferenceAdapter:
    """
    Generic OpenAI-compatible pooled inference adapter.

    Takes server_urls and creates ServerPool internally.
    Orchestration layers should not know about the internal pooling.

    Works with any backend that exposes /v1/models and /v1/chat/completions
    (LM Studio, llama-server, vLLM, Ollama, etc.).
    """

    def __init__(
        self,
        server_urls: list[str | dict],
        api_key: Optional[str] = None,
        fallback_api_key_env: str = "LMSTUDIO_API_KEY",
    ):
        """
        ...
        Args:
            server_urls: List of server URLs (str) or config dicts
                         with {"url": ..., "api_key": ...}.
            api_key: Global auth token fallback.
                     Used when a server has no per-server key.
            fallback_api_key_env: Environment variable name for global API key
                                  fallback (default: LMSTUDIO_API_KEY).
        """
        self._api_key = api_key or os.environ.get(fallback_api_key_env)
        configs = []
    ...
    async def stream_completion(self, task: InferenceTask) -> AsyncGenerator[str, None]:
        """
        Stream completion from an OpenAI-compatible server using InferenceTask.
        """
...
async def stream_completion(
    ...
    """
    Stream completion from a specific OpenAI-compatible server.
    ...
    async with httpx.AsyncClient(timeout=timeout_seconds) as client:
        async with client.stream(
            ...
            if response.status_code >= 400:
                ...
                raise LocalInferenceError(f"Inference error for {model_id}: {msg}")
            ...
            if not stream_done and not has_content:
                raise LocalInferenceError(f"Stream aborted for {model_id} (no [DONE], no content)")
