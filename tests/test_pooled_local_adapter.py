...
class TestGetAvailableModels:
    ...
    async def test_both_servers_unreachable_raises(self, two_server_urls):
        """Both servers unreachable → raises LocalInferenceError (empty manifest fast-fail)."""
        ...
        adapter = PooledLocalInferenceAdapter(two_server_urls)
        with pytest.raises(LocalInferenceError, match="No models available"):
...
class TestToolCallSentinel:
    ...
    async def test_tool_calls_yield_sentinel(self):
        ...
        adapter = PooledLocalInferenceAdapter([server_url])
        await adapter.get_available_models()
...
class TestApiKeyAuth:
    ...
    def test_api_key_from_constructor(self):
        ...
        adapter = PooledLocalInferenceAdapter(["http://server0:1234"], api_key="explicit-key")
    ...
    def test_api_key_fallback_to_env(self, monkeypatch):
        ...
        adapter = PooledLocalInferenceAdapter(["http://server0:1234"])
    ...
    async def test_empty_manifest_raises(self):
        """All servers return empty manifests → raises LocalInferenceError immediately."""
        from prompt_prix.adapters.pooled_local import LocalInferenceError
    ...
    async def test_401_raises_auth_error_message(self):
        """401 response → LocalInferenceError with actionable auth message."""
        from prompt_prix.adapters.pooled_local import LocalInferenceError
        ...
        with pytest.raises(LocalInferenceError, match="Authentication failed"):
            async for _ in adapter.stream_completion(task):
                pass
...
class TestDispatcherExceptionWrapping:
    """Tests that l-i-p dispatcher exceptions are wrapped as LocalInferenceError."""

    @pytest.mark.asyncio
    @respx.mock
    async def test_dispatcher_timeout_raises_lmstudio_error(self, two_server_urls):
        """DispatcherTimeoutError from submit() → LocalInferenceError."""
        from prompt_prix.adapters.pooled_local import LocalInferenceError
        ...
        with pytest.raises(LocalInferenceError, match="Timed out"):
            async for _ in adapter.stream_completion(task):
                pass
    ...
    async def test_no_models_available_raises_lmstudio_error(self, two_server_urls):
        """NoModelsAvailableError from submit() → LocalInferenceError."""
        from prompt_prix.adapters.pooled_local import LocalInferenceError
...
class TestHealthFeedback:
    ...
    async def test_connect_error_reports_server_dead(self):
        ...
        with pytest.raises(LocalInferenceError, match="failed during streaming"):
            async for _ in adapter.stream_completion(task):
                pass
